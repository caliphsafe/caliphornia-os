...
type WeatherData = {
  current: { temperature: number | null; label: string };
  today: { tempMax: number | null; tempMin: number | null; label: string };
};
  ...
  try {
    ...
    return {
      ...
      today: {
        ...
        label: weatherCodeLabel(data?.daily?.weather_code?.[0] ?? data?.current?.weather_code),
      },
    };
  } catch {
    return null;
...
export default async function MiliaPage() {
  ...
  const cards = await Promise.all(
    songs.map(async (song) => {
      const [weather, access] = await Promise.all([
        getWeather(song),
        getSongPlaybackAccess({ userEmail: session.email, projectSlug: "milia", song }),
      ]);

      return { song, weather, access };
    })
  );

  const queue: GlobalTrack[] = cards.map(({ song, access }) => ({
    id: song.id,
    songId: song.id,
    slug: song.slug,
    songSlug: song.slug,
    ...
    description: access.lockedReason || song.description || undefined,
    playlistSongSlug: song.slug,
    analyticsSongSlug: song.slug,
    sourceApp: "milia",
    isPreview: access.isPreview,
    clipStartSeconds: access.clipStartSeconds,
    clipEndSeconds: access.clipEndSeconds,
  }));
  ...
  return (
    <main className={styles.page}>
      <div className={styles.chrome}>
        ...
        <div className={styles.morePill}>Milia</div>
      </div>

      <section className={styles.container}>
        <div className={styles.hero}>
          <h1 className={styles.heroTitle}>Milia</h1>
          <p style={{ margin: "10px 0 0", color: "rgba(255,255,255,.78)", fontSize: 18 }}>
            Music organized like forecasted emotional weather.
          </p>
        </div>
      </section>
      ...
      <section className={`${styles.container} ${styles.stack}`}>
        {cards.length ? (
          ...
          <section className={styles.panel}>
            <h2 className={styles.panelTitle}>Milia is being restored.</h2>
            <p className={styles.detailIntro}>Add Milia songs with source_app_slug set to milia to populate the Weather-style experience.</p>
          </section>
        )}
      </section>
    </main>
