...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await supabaseAdmin.from("user_favorite_songs").select("id,status").or(`user_id.eq.${user.id},user_email.eq.${user.email}`).eq("song_id", song.data.id).maybeSingle();
    ...
    const maxOrder = await supabaseAdmin.from("user_favorite_songs").select("favorite_order").eq("user_id", user.id).neq("status", "removed").order("favorite_order", { ascending: false }).limit(1);
    const nextOrder = Number(maxOrder.data?.[0]?.favorite_order || 0) + 1;
    await supabaseAdmin.from("user_favorite_songs").upsert(
      { user_id: user.id, user_email: user.email, song_id: song.data.id, song_slug: song.data.slug, status: "active", source_type: "manual", favorite_order: nextOrder, removed_at: null },
